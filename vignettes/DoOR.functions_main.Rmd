...
DoOR consists of two R packages and both are needed for DoOR to work properly. One package, `DoOR.data` contains all the _Drosophila_ odor responses we gathered from labs around the world or recorded ourselves. The other package `DoOR.functions` contains the DoOR framework for integrating heterogeneous data sets as well as analysis tools and functions for visalization. 
...
## Importing new data with `importNewData()`
...
## Building the complete data base with `CreateDatabase()`
...
## Updating parts of the data base with `updateDatabase()`
...
## Removing a study with `removeStudy()`
...
## Updating the odor information with `updateOdorInfo()`
If we edit the general odor information in `DoOR.data::odor` we need to update all other DoOR objects with the new information. `updateOdorInfo()` overwrites the first 5 column of the DoOR responding units data frames (e.g. `Or22a`), it does not add or remove lines!
...
## `transID()`
Maybe **the** most important little function in DoOR. With `transID()` we can translate odorant identifiers, e.g. from CAS numberst to InChIKeys or to Names. The information is taken from `DoOR.data::odor`, any `colnames(odor)` can be used to define input or output:
```{r}
transID("123-92-2")
