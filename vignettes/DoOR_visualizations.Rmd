...
---
Apart from merging data and performing calculations on it, the `DoOR.functions` package provides several ways to visualize the data served by the `DoOR.data` package. Please see the [DoOR.function vignette](DoOR.functions.html) for instructions on how to install and load both functions and data. The plotting functions can be identified by their common prefix `DoORplot_`, most of them make use of the `ggplot2` package which makes it possible to override most all of the theming options.

## Content
* [Loading data](#loading)
* [Visualizing odorants _vs_ responding units with `DoORplot_responseMatrix()`](#responseMatrix)
* [Visualizing ensemble responses with `DoORplot_ALmap()`](#ALmap)
* [Visualizing tuning curves with `DoORplot_tuningCurve()`](#tuningCurve)
* [Visualizing response profiles `DoORplot_responseProfile()`](#responseProile)
* [Comparing response profiles `DoORplot_compareProfiles()`](#compareProfiles)
* [Visualizing odorant responses across responding units with `DoORplot_acrossReceptors()`](#acrossReceptors)
* [Visualizing odorant responses across OSNs with `DoORplot_acrossOSNs()`](#across_OSNs)
...
### Odorants
`DoORplot_tuningCurve()` can as well be used to visualize the ensemble of responding units a given odorant activates. Therefore we specify an odorant name instead of a response unit name:
```{r}
...
As mentioned, all of these plot are generated with the ggplot2 package which makes it possible to override theming:
```{r}
library(ggplot2)
...
## Comparing response profiles `DoORplot_compareProfiles()`{#compareProfiles}
As the name indicates, with `DoORplot_compareProfiles()` we can plot two response profiles side by side for comparison. The syntax here differs from the prvious plots as we can use it also to compare the original data sets in DoOR.
...
We see that the overlapping responses of these two data sets that both measures Or22a sensory neurons are in good accordance.
...
As this plot gets pretty messy, we have the option to restrict plotting to certain subsets of sensilla. We can for example only plot t he responses of antennal basiconic (ab) sensilla:
