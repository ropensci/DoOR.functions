...
#'   TRUE)
#' @param door_excluded_data the data frame that contains the list of excluded
#'   data sets.
#' @param plot logical
  ...
  function(receptor,
    ...
    if (permutation == TRUE) {
      ...
      if (length(da) > door_default_values("num.charColumns")) {
        if (missing(perm)) {
          perm <-
            matrix(studies[permutations(length(studies))], ncol = length(studies))
          message(
            paste(
              "All possible permutations (",
              dim(perm)[1],
              ") have been calculated, now merging.",
              sep = ""
            )
          )
        }
        ...
        # compute the mean correlation between merged responses and original
        # response.
        meanCorrel <- matrix(NA, nrow = dim(perm)[1])
        
        for (i in 1:dim(perm)[1]) {
          merge.try <-
            try(model_response_seq(
              data = da,
              SEQ = perm[i, ],
              overlapValues = overlapValues,
              select.MDValue = select.MDValue,
              strict = strict,
              plot = plot
            ),
            silent = TRUE)
          ...
          message(paste(
            ...
            dim(perm)[1],
            "] ",
            paste(perm[i, ], collapse = ", "),
            " ------ Mean distance: ",
            round(meanCorrel_merge.try, 4),
            sep = ""
          ))
        }
        ...
        perm_MC <-
          data.frame(perm, meanCorrel)
        # data frame, the last column contains the mean correlation values.
        ...
        message(paste(
          "The optimized sequence with the lowest mean MD",
          round(meanCorrel[min.MD], 4),
          "is:"
        ))
    ...
    merged_data_withInChIKey <-
      data.frame(InChIKey = da$InChIKey, merged_data = merge)
    matchInChIKey <-
      match(merged_data_withInChIKey$InChIKey,
            rownames(response_matrix_nn))
    findNA_InChIKey <- which(is.na(matchInChIKey))
    if (!is.na(findNA_InChIKey[1])) {
      addRow <-
        matrix(NA,
               nrow = length(findNA_InChIKey),
               ncol = dim(response_matrix_nn)[2])
      colnames(addRow) <- colnames(response_matrix_nn)
      rownames(addRow) <-
        merged_data_withInChIKey[findNA_InChIKey, "InChIKey"]
    ...
    assign(
      ...
      as.data.frame(response_matrix_nn, stringsAsFactors = FALSE),
      envir = .GlobalEnv
    )
    message(paste(
      "door_response_matrix_non_normalized has been updated for",
      receptor
    ))
    ...
    mp_orx       <-
      match(colnames(da)[recordColumn], responseRange[, "study"])
    Rmax         <-
      apply(as.matrix(da[, recordColumn]), 2, function(x)
    ...
    merged_data_withInChIKey <-
      data.frame(InChIKey = da$InChIKey, merged_data = merged_data)
    matchInChIKey <-
      match(merged_data_withInChIKey$InChIKey,
    ...
    door_excluded_data$excluded <-
      as.character(door_excluded_data$excluded)
    door_excluded_data[door_excluded_data$OR == receptor, "excluded"] <-
