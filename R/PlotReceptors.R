...
#  data 	: data frame; containing the names of odorant receptors, CAS number(s) and normalized response values. 
#  odor.data 	: data frame; chemical data of odors. 
#  tag 		: character string; default tag is the CAS number. 
...
#  file.name 	: character string; file name; if missing, the default file name is "PlotReceptors.pdf" or "PlotReceptors.jpeg" 
...

{
	...
	if (tag=="Name") 
	{
		matchOdor <- match(data[,"Odor"], odor.data$CAS)
	...
	if (JPEG==TRUE) 
	{
		if (missing(file.name)) { jpeg("PlotReceptors.jpeg",width=pic.width, height=pic.height,pointsize=point.size) }
		...
		if (PDF==TRUE)	
		{
			if (missing(file.name)) { pdf("PlotReceptors.pdf",width=6, height=8) }
