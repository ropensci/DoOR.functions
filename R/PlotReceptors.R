...
#' odor receptors to odors. For \code{PlotReceptors}, the given data should be
#' generated by \code{\link{findRespNorm}} or the columns of data must be name
#' of odor receptors, CAS number and normalized response value. The color bar
...
#' 
#' @param data data frame; containing the names of odorant receptors, CAS
#' number(s) and normalized response values.
#' @param odor.data data frame; chemical data of odors.
#' \code{\link[DoOR.data]{odor}}
#' @param tag character string; default tag is the CAS number.
...
#' name is "PlotReceptors.pdf" or "PlotReceptors.jpeg"
...
#' result2<-findRespNorm(cas,responseMatrix = response.matrix)
#' PlotReceptors(result2,odor.data=odor,tag="Name",PDF=TRUE)
#' 
PlotReceptors <-
