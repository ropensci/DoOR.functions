...
door_default_values <- function(DoOR_default) {
  ...
  if (DoOR_default == "select.MDValue") 	{
    return(0.1415)
  } # select_model.R, model_response.R, create_door_database.R, 
    # create_door_database.R
  if (DoOR_default == "points.cex") 	    {
    return(1)
  } # project_points.R
  if (DoOR_default == "cex.title") 	      {
    return(1)
  } # project_points.R
  if (DoOR_default == "dot.size") 	      {
    return(3)
  } # dplot_response_matrix.R
  if (DoOR_default == "cex.labels") 	    {
    return(1)
  } # dplot_response_matrix.R
  
  
  ## integer
  if (DoOR_default == "overlapValues") 	  {
    return(5)
  } # select_model.R
  if (DoOR_default == "num.charColumns") 	{
    return(5)
  } # create_door_database.R, model_response.R, import_new_data.R, 
    # model_response_seq.R
  
  ## logical
  if (DoOR_default == "glob.normalization") {
    return(TRUE)
  } # model_response.R
  if (DoOR_default == "merged") 	       	  {
    return(TRUE)
  } # select_model.R
  if (DoOR_default == "select.MD") 	        {
    return(TRUE)
  } # calculate_model.R, project_points.R
  if (DoOR_default == "closest") 		        {
    return(TRUE)
  } # LLSIest.R
  if (DoOR_default == "scalebar")   	      {
    return(TRUE)
  } # dplot_al_map.R
  if (DoOR_default == "title") 		          {
    return(FALSE)
  } # project_points.R
  if (DoOR_default == "plot") 		          {
    return(FALSE)
  } # model_response.R, project_points.R
  
  ## default null
  if (DoOR_default == "main") {
    return(NULL)
  } # dplot_al_map.R
  
  ## character
  if (DoOR_default == "zero") 		    {
    return("SFR")
  } # get_normalized_responses.R
  if (DoOR_default == "tag.ALmap") 	{
    return("glomerulus")
  } # dplot_al_map.R
  if (DoOR_default == "tag") 		      {
    return("InChIKey")
  } # dplot_response_profile.R, dplot_compareReceptors.R, create_door_database.R
  if (DoOR_default == "ident")        {
    return("InChIKey")
  } # import_new_data.R
  if (DoOR_default == "color.receptor") {
    return("#5A9BD4")
  }  # dplot_tuningCurveReceptor
  if (DoOR_default == "color.odorant")  {
    return("#7AC36A")
  }  # dplot_tuningCurveOdorant
  
  
  ## character vector
  if (DoOR_default == "colors")   {
    return(c(
      "#0570b0",
      "#74a9cf",
      "#ffffff",
  ...
  } # dplot_al_map.R, dplot_response_matrix.R, dplot_response_profile.R, 
    # dplot_compareReceptors.R
  if (DoOR_default == "interval.X") 	{
    return(c(-1, 2))
  } # compute_MD.R, modelfunction.R, project_points.R
  ...
  } # export_door_data.R, import_new_data.R, update_door_database.R, 
    # model_response.R
  ...
  } # export_door_data.R, get_responses.R, import_new_data.R, 
    # update_door_database.R, model_response.R
