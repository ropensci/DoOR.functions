...
dplot_al_map <- function(InChIKey,
  ...
  if (!requireNamespace("ggplot2", quietly = TRUE))
    stop(
      "ggplot2 is required for AL map plotting, please install via 
       install.packages('ggplot2')",
      call. = FALSE
    )
  if (!requireNamespace("grid", quietly = TRUE))
    stop(
      "grid is required for AL map plotting, please install via 
  ...
  response.data <-
    get_normalized_responses(InChIKey, zero = zero, 
                             response_matrix = response_matrix)
  
  plotdata <- door_AL_map[["glomeruli"]]
  plotdata$receptor <-
    door_mappings$receptor[match(plotdata$glomerulus, door_mappings$code)] 
  # match mapped glomeruli and extract receptor names
  plotdata$response <-
    response.data$Response[match(plotdata$receptor, response.data$ORs)] 
  # pick responses from response.data
  
  labels <- door_AL_map$labels
  labels <-
    cbind(labels, door_mappings[match(labels$glomerulus, door_mappings$code), 
                      c("receptor", "sensillum", "OSN", "co.receptor", "Ors")])
  ...
  p <- ggplot2::ggplot(data = plotdata) +
    ggplot2::geom_polygon(
      data = door_AL_map$background,
      ggplot2::aes(x = x, y = y, group = group),
      fill = "grey80",
      color = "grey80"
    ) +
    ggplot2::geom_polygon(
      data = door_AL_map$bg.cutout,
      ggplot2::aes(x = x, y = y, group = group),
      fill = "white",
      color = "white"
    ) +
    ggplot2::geom_polygon(
      data = door_AL_map$unmapped_not.olf,
      ggplot2::aes(x = x, y = y, group = glomerulus),
      fill = "grey45",
      color = "grey75"
    ) +
  ...
  if (legend == TRUE) {
    ...
    p <- p +
      ...
      ggplot2::annotate(
        ...
        y = y + lsize / 2 ,
        label = c("unmapped", "NA", "background"),
        hjust = 0,
        size = .3 * base_size
      )
  }
