...
estimate_missing_value <-
  function(data, nodor, method = "PC") {
    ...
    precentageMissing <-
      round(dim_whichmissing[1] / length(inputdata), 4) * 100
    message(paste(
      "There are",
      dim_whichmissing[1],
      paste("(", precentageMissing, "%)", sep = ""),
    ...
    for (i in 1:dim_whichmissing[1]) {
      ...
      if (inherits(tryestmatine, "try-error")) {
        print(
          paste(
            "The odorant response of",
            receptor,
            "to",
            inchikey,
        ...
        outputdata[inchikey, receptor] <- tryestmatine$estimation
      }
      if (inherits(tryestmatine, "Warning messages:")) {
        stop
      }
    }
    whichmissingOutput 	<- which(is.na(outputdata), arr.ind = TRUE)
    dim_whichmissingOutput 	<- dim(whichmissingOutput)
    precentageMissingOutput <-
      round(dim_whichmissingOutput[1] / length(outputdata), 4) * 100
    N_estimate <- dim_whichmissing[1] - dim_whichmissingOutput[1]
    message(
      paste(
        "There are",
        dim_whichmissingOutput[1],
        paste("(", precentageMissingOutput, "%)", sep = ""),
        "missing values in the output data."
      )
    )
