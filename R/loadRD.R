...
loadRD <- function(odorantReceptors = default.val("ORs"), supportData = TRUE) {
	...
	supportData.name <- c("AL256", "data.format", "glo.dist", "odor", "DoOR.mappings", "ORs", "reference","response.matrix", "response.range","unglobalNorm_response.matrix","weight.globNorm","dataset.info")
