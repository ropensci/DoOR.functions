...
loadRD <- function(odorantReceptors = default.val("ORs"), supportData = TRUE) {
	...
	supportData.name <- c("AL.map", "data.format", "excluded.data", "glo.dist", "odor", "DoOR.mappings", "ORs", "reference","response.matrix", "response.range","response.matrix_non.normalized","weight.globNorm","dataset.info")
