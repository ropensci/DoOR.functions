...
#' @param sub character, a subset of responding units returned response matrix
#' @param threshold.p numeric, a p-value threshold, only correlations below will
#'   be returned
#' @param threshold.cor numeric, a correlation-coefficient threshold, only
...
map_receptor <- function(data,
                 ...
                 threshold.cor,
                 nshow) {
  if (!("odorants" %in% colnames(data)) |
      !("responses" %in% colnames(data)))
    stop("Please provide InChIKeys and responses in columns named 'odorants' and
         'responses'.")
  ...
  res             <- data.frame()
  response_matrix <-
    response_matrix[match(data$odorants, rownames(response_matrix)),]
  
  if (!missing(sub))
    response_matrix <-
    response_matrix[, match(sub, colnames(response_matrix))]
  ...
  if (length(n) > 0) {
    message(paste(
      "skipped ",
      paste(names(response_matrix)[n], collapse = ", "),
      " as overlap (n) was < 3",
      sep = ""
    ))
