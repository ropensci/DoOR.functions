...
# performs the merging of all studies (that measured a specific receptor) into a common response model for this receptor 
# uses a heuristic (as described in the DoOR paper) to estimate a good merging sequence.

# input parameters:
####################

# da         		: data frame; for all studies and a selected receptor (selected manually or by CreateDatabase() that calls modelRP for all receptors); 
#                         the first four columns indicate odor class, odor name, compound ID (CID) and CAS number. 
#                         The response data can be obtained by data() e.g. data(Or35a) or loadRD() to load all response data. 
# select.MDValue 	: numeric; threshold on the MD for rejecting a fit. Default of 1.414 corresponds to no threshold because MD on normalized data can never be larger than sqrt(2)
# overlapValues 	: numeric; a criterion using to refuse a data set that has not enough overlap value.
# responseRange 	: data frame; contains response ranges for all studies (also ones that do not contribute to "data" --> needed for global normalisation)
# weightGlobNorm 	: data frame; a binary data matrix; 1 indicates given odor has been measured in the given study, NA indicates NOT.
# glob.normalization 	: logical; if TRUE  --> perform a global normalization (see subfunction globalNorm() ), if FALSE --> local normalization to interval [0,1] (see "projectPoints.R" )              
# plot       		: logical; the merging process is plotted (like in Figure 2 in the paper)


# output is a list containing two character vectors "Source.data" (studies that have been merged) and "Excluded.Data" (studies that have been excluded), 
# a data frame "model.response" ( consensus responses )

{
    ...
    if (!is.data.frame(da))  #safety check for input data format
    {
	...
	###
    	...
    	projected <- projectPoints(x = pda[,selected$selected.x], y = pda[,selected$selected.y], best.model = selected$best.model, 
                                   xlab = selected$selected.x,    ylab = selected$selected.y,    plot = plot, title = plot)
   	...
   	merged_data[projected$Double.Observations$ID] <- projected$Double.Observations$NDR
		...
		if ( (names(selected.next$best.model) == "initial") | (names(selected.next$best.model) == "no.fitted.model") | (selected.next$best.model[[1]]$MD > select.MDValue) )
		   ...
		   ind <- 0
    		...
    		projected <- projectPoints(x = pda[,selected.next$selected.x], y = merged_data, best.model = selected.next$best.model,
                                   xlab = selected.next$selected.x,    ylab = selected.next$selected.y,    plot = plot, title = plot)
