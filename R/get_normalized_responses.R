...
#' @param response_matrix a data frame, as e.g. "door_response_matrix" that is
#'   loaded by \code{\link{model_response}}. It is also possible to create this
#'   frame manually using \code{\link{model_response}}.
#' @param round numeric, round to this amount of digits, set to NA if you do not
#'   want to round
...
get_normalized_responses <- function(odors,
  ...
  if (any(is.na(mp))) {
    stop(
      paste("The following odorants are not in the database: "),
      paste(odors[which(is.na(mp))], collapse = ", ")
    )
  }
  ...
  res <-
    data.frame(
      ORs      = rep(colnames(response_matrix), each = length(odors)),
