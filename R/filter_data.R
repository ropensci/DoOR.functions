# filter_data
# 
# Filter DoOR datasets before merging (remove studies with few overlapping
# points or SD = 0). This function is used by update_door_database to pre filter
# before performing all possible merges.
# 
...
#   
# @return a list containing the filtered data set and a data frame naming the
#   removed data sets and the reason for removal
#   
# @examples
# library(DoOR.data)
...
filter_data <-
  function(data,
           ...
           charColumns = door_default_values("num.charColumns")) {
    ...
    if (any(study.sd == 0)) {
      message(paste(receptor, ": REMOVED ", studies[which(study.sd == 0)], 
                    " as it had a SD of 0.\n", sep = ""))
      excluded <-
        data.frame(study = studies[which(study.sd == 0)], reason = "SD = 0")
      data <-
        data[, -which(names(data) == studies[which(study.sd == 0)])]
      recordColumn  <- as.numeric(c((charColumns + 1):dim(data)[2]))
      studies       <- names(data)[recordColumn]
    }
    ...
    while (checkOverlap == TRUE & length(studies) > 1) {
      ...
      for (i in 1:dim(tmp)[2]) {
        a <- tmp[, i]
        b <-
          apply(as.data.frame(tmp[, -i]), 1, function(x)
            any(x == TRUE))
      ...
      if (any(ol < overlapValues)) {
        ...
        message(
          paste(
            ...
            ": REMOVED ",
            remove,
            " as overlap with all other studies was smaller than ",
            overlapValues,
            "!\n",
            sep = ""
          )
        ...
        excluded <-
          rbind(excluded, data.frame(
            study = remove,
            reason = paste("overlap <", overlapValues)
          ))
      } else {
        checkOverlap <- FALSE
      }
