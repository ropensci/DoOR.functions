...
test_that("model response output is numeric", {
  expect_that(model_response(Or35a)$model.response$merged_data,
              is_a("numeric"))
})
...
test_that("reset_sfr output has same dimenstions as input", {
  expect_that(dim(reset_sfr(door_response_matrix, "SFR")), 
              is_identical_to(dim(door_response_matrix)))
})
...
test_that("select_model produces a list", {
  ...
  expect_that(
    select_model(
      candidate = studies,
      data_candidate = norm_data_candidate,
      merged = FALSE
    ),
    is_a("list")
  )
...
test_that("calculate_model returns a list", {
  x <- door_norm(Or35a[, 6])
  y <- door_norm(Or35a[, 9])
  expect_that(calculate_model(x, y, 
                              select.MD = door_default_values("select.MD")),
              is_a("list"))
  
})

test_that("update_database with permutation = TRUE 
           updates all door data objects",
          {
            load_door_data(nointeraction = TRUE)
...
test_that("update_database with permutation = FALSE
           updates all door data objects",
          {
            load_door_data(nointeraction = TRUE)
            tmp_a <- door_response_matrix
